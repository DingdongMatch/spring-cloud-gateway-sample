package com.match.gateway.predicate;
...
public class CustomReadBodyRoutePredicateFactory extends AbstractRoutePredicateFactory<CustomReadBodyRoutePredicateFactory.Config> {
    ...
    public Predicate<ServerWebExchange> apply(Config config) {
        throw new UnsupportedOperationException("ReadBodyRoutePredicateFactory is only async");
    }
    ...
    public AsyncPredicate<ServerWebExchange> applyAsync(Config config) {
        return new AsyncPredicate<ServerWebExchange>() {
            ...
            public Publisher<Boolean> apply(ServerWebExchange exchange) {
                ...
                if (cacheBody != null) {
                    try {
                        boolean test = config.getPredicate().test(cacheBody);
                    ...
                    return ServerWebExchangeUtils.cacheRequestBodyAndRequest(exchange, serverHttpRequest -> {
                        return ServerRequest.create(exchange.mutate().request(serverHttpRequest).build(), CustomReadBodyRoutePredicateFactory.this.messageReaders)
